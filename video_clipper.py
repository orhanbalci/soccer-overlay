...
def clip_video(input_path, start_time, end_time, output_path=None):
    ...
    if start_seconds >= video.duration:
        raise ValueError(f"Start time ({format_time(start_seconds)}) is beyond video duration ({format_time(video.duration)})")
    
    if end_seconds > video.duration:
        print(f"Warning: End time ({format_time(end_seconds)}) is beyond video duration ({format_time(video.duration)})")
